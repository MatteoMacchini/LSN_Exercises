...
int main () {
    ...
    for (int i=1; i<=neq; i++) {
        // Eq << i << " " << walker[iv]/(double)npart + vtail << " " << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl;
        Move();
        Measure();
    }
    ...
    /*int M = 500000;
    ofstream Ac("ac.out");
    if (!Ac) cerr << "Non posso creare il file ac.out" << endl;
    
    for (int i=1; i<=M; i++) {
        Ac << i << " " << walker[iv]/(double)npart + vtail << " " << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl;
        Move();
        Measure();
...
void Averages (int iblk) //Print results for current block
{
    ...
    for (int i=igofr; i<n_props; ++i) {
        r = (i-igofr)*bin_size;
        gdir = 3.*blk_av[i]/(4.*M_PI*rho*npart*blk_norm*(pow(r+bin_size,3)-pow(r,3)));
        glob_av[i] += gdir;
        glob_av2[i] += gdir*gdir;
        err_gdir = Error(glob_av[i],glob_av2[i],iblk);
        Gofr << gdir << " ";
        if(iblk==nblk) Gave << (i-igofr+0.5)*bin_size << " " << glob_av[i]/(double)iblk << " " << err_gdir << endl;
    }
    Gofr << endl;
    cout << "----------------------------" << endl << endl;
