...
double variationalWaveFunction_second(double x)
{
    //double mu = 0.806, sigma = 0.619;
	return 0.;
    //return pow(x-mu,2)/pow(sigma,4)*exp(-pow(x-mu,2)/(2*sigma*sigma)) + pow(x+mu,2)/pow(sigma,4)*exp(-pow(x+mu,2)/(2*sigma*sigma)) - (exp(-pow(x-mu,2)/(2*sigma*sigma)) + exp(-pow(x+mu,2)/(2*sigma*sigma)))/pow(sigma,2);
}
