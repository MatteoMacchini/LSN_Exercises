...
void Chromosome :: Circle () {
    vector<double> x;
    vector<double> y;
    
    ofstream fout("init_circ.dat"); // stampo su file il cammino di partenza
    if (!fout) cerr << "Non posso creare il file init_circ.dat" << endl;
        
    for (int i=0; i<m_N; i++) {
        double alpha = m_rand->Rannyu(0.,2*M_PI);
        x.push_back(m_r*cos(alpha));
        y.push_back(m_r*sin(alpha));
        fout << i << " " << m_r*cos(alpha) << " " << m_r*sin(alpha) << endl;
    }
