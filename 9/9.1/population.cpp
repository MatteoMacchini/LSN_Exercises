...
void Population :: First_Pop_Circle (unsigned int N, double r) {
    m_c = true;
...
void Population :: New_Pop () {
    ...
    for (int k=0; k<(int)m_pop.size()/2; k++) {
        ...
        double p_m = 0.05;
        // Mutations
        if (m_rand->Rannyu()<p_m) f1.Swap();
        if (m_rand->Rannyu()<p_m) f2.Swap();
        
        if (m_rand->Rannyu()<p_m) f1.Shift();
        if (m_rand->Rannyu()<p_m) f2.Shift();
        
        if (m_rand->Rannyu()<p_m) f1.Inversion();
        if (m_rand->Rannyu()<p_m) f2.Inversion();
        
        if (m_rand->Rannyu()<p_m) f1.Cycle();
        if (m_rand->Rannyu()<p_m) f2.Cycle();
        
        if (m_rand->Rannyu()<p_m) f1.Swap_Sequence();
        if (m_rand->Rannyu()<p_m) f2.Swap_Sequence();
...
void Population :: Print_on_File () {
    if (m_c == true) {
        ofstream fout("final_pop_circle.out");
        if (!fout) cerr << "Non posso creare il file final_pop_circle.out" << endl;
        ...
        ofstream fout("final_pop_square.out");
        if (!fout) cerr << "Non posso creare il file final_pop_square.out" << endl;
        for (int i=0; i<m_n_chrom; i++) {
            int N = m_pop[i].Get_N();
            vector<int> chrom = m_pop[i].Get_chrom();
            for (int j=0; j<N; j++) {
